...
class Scenario(BaseScenario):
    ...
    def observation(self, agent, world):
        ...
        for other in world.agents:
            if not agent.movable and other.silent:
                max_len = self.observation(other,world).shape[0]
            if other is agent or (other.state.c is None):
                continue
            comm.append(other.state.c)
        # speaker
        if not agent.movable:
            return_ = np.concatenate([goal_color])
            return np.pad(return_, (0, max_len - len(return_)), mode='constant', constant_values=0)
        # listener
        if agent.silent:
            return np.concatenate([agent.state.p_vel] + entity_pos + comm)
        # Next: Change the action space as commented in the mappo/onpolicy/envs/mpe/environment.py.